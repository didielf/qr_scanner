...
import 'package:qr_scanner/src/qr_scanner_animation.dart';
import 'package:qr_scanner/src/qr_scanner_overlay_animation_theme.dart';
...
class QRView extends StatefulWidget {
  const QRView({
    ...
    this.overlay,
    this.scannerTheme,
    this.showScannerAnimation = false,
  ...
  /// Use [scannerTheme] to style scanner animation
  ...
  /// Use [showScannerAnimation] to specify whether or not the scanner animation should show.
  final bool showScannerAnimation;
...
class _QRViewState extends State<QRView> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animationController.addStatusListener((status) {
      if (widget.overlay!.isInitialize) {
  ...
  Widget _getPlatformQrViewWithOverlay() {
    ...
    if (widget.showScannerAnimation) {
      items.add(Container(
        child: ImageScannerAnimation(
          _animationController,
          widget.overlay!,
          scannerTheme: widget.scannerTheme ??
              QrScannerOverlayAnimationTheme(
                  color: widget.overlay!.borderColor),
        ),
      ));
    }

    return Stack(children: items.map<Widget>((e) => e).toList());
  }
